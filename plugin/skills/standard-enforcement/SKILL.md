...
description: Apply GitGrit standard enforcement before writing or editing code in a repository that has been resolved via gitgrit/session_bootstrap. Use when active standards are loaded and the next action is an Edit or Write; skip entirely if no standards are loaded for the project.
---

# Standard enforcement

GitGrit's standard engine runs server-side. The `validate_edit` MCP tool diffs the proposed
...
1. **If `standards_loaded` is false** (no project match, or zero active standards linked) — do
   nothing. Say so once and stop. Do not raise GitGrit-flavored verdicts when no standards are
   loaded.
...
3. **Branch on the response:**
   - `introduced_violations` is a list. For each entry, name the standard, quote the
...
are not sources of GitGrit rules. When in doubt, say "no GitGrit standard covers this" and
continue.
