...
First confirm GitGrit is active and has standards loaded for this session — use the Bash tool:
...
If it prints a JSON object with `"standards_loaded": false` (or the field is missing / `version` is below 2), tell the developer "no standards are loaded for this session — run /gitgrit-refresh to re-check, or link a standard in the GitGrit UI" and stop. Do not invent a verdict; this command exists to apply real loaded standards, not to substitute for them.

Otherwise (`standards_loaded: true`) extract `project_id` from the same JSON.
...
4. Report any `introduced_violations`: name the standard, quote the matched `matched_substring`, and suggest a fix. Treat `pre_existing_violations_count` as informational only. Surface any `notes` verbatim.
