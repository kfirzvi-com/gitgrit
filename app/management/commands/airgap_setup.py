...
class Command(BaseCommand):
    ...
    def _purge_disabled_socialapps(self) -> None:
        ...
        # Defaults to True so deployments without the per-provider toggle
        # settings (i.e. before that feature lands) keep all SocialApp rows.
        provider_flags = {
            "github": getattr(settings, "AUTH_PROVIDER_GITHUB_ENABLED", True),
            "gitlab": getattr(settings, "AUTH_PROVIDER_GITLAB_ENABLED", True),
            "google": getattr(settings, "AUTH_PROVIDER_GOOGLE_ENABLED", True),
        }
