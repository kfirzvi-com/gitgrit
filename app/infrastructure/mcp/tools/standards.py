...
async def list_standards() -> list[dict]:
    """List all standards in the current workspace."""
    tenant = get_auth().tenant
    return await sync_to_async(_service.list_standards)(tenant)


@register
async def get_standard(standard_id: str) -> dict:
    """Get full details of a standard, including its Python code and recent executions."""
    tenant = get_auth().tenant
    return await sync_to_async(_service.get_standard)(tenant, standard_id)
...
async def create_standard(
    ...
    return await sync_to_async(_service.create_standard)(tenant, user, {
        "name": name,
        "code": code,
...
@register
async def delete_standard(standard_id: str) -> dict:
    """Permanently delete a standard."""
    tenant = get_auth().tenant
    await sync_to_async(_service.delete_standard)(tenant, standard_id)
    return {"deleted": True, "standard_id": standard_id}


@register
async def set_standard_code(
    standard_id: str,
    file_path: str,
    change_summary: str = "Code updated via file",
) -> dict:
    """Load standard code from a local file and save it to a standard.

    Use this instead of passing code inline to update_standard() when the standard
    ...
    Workflow:
        1. Write the standard code to a local file using the Write tool
           (e.g. /tmp/standard.py)
    ...
    return await sync_to_async(_service.update_standard)(tenant, user, standard_id, {
        "code": code,
